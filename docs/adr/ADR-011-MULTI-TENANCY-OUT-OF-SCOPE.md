...
- File uploads are shared across the single tenant by default
  (`FILE_USER_SCOPED_FILES=false`); deployments that need per-user media can
  set it to `true`. This is a file-library policy, not a tenant scope.
