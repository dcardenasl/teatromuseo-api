...
- Los uploads de archivos se comparten por defecto dentro del tenant único
  (`FILE_USER_SCOPED_FILES=false`); una instalación que necesite media por
  usuario puede activarlo con `true`. Esto es una política de biblioteca de
  archivos, no un scope de tenant.
